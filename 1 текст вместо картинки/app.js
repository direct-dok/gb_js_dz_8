...
function handleClick(clickedButtonEvent) {
    ...
    if (card.button.innerText == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        ...
        showMoreText(card);
    } else if (card.button.innerText != 'Подробнее') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let text = 'A simple MVC framework created in PHP. Contribute to skdev/PHP-MVC-Framework development by creating an account on GitHub.';
